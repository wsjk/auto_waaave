...
vec2 rotate(vec2 coord, float theta) {
  ...
  vec2 center_coord = vec2(coord.x - 0.5, coord.y - 0.5);
  ...
  rotate_coord = rotate_coord + vec2(0.5, 0.5);
  return rotate_coord;
...
void main() {
  ...
  vec3 input1ColorHsb = rgb2hsb(
    vec3(input1Color.r, input1Color.g, input1Color.b)
  );
 ...
 */
  ...
  vec2 fbCoord = vec2(texCoordVarying.x - 0.5, texCoordVarying.y - 0.5);
  ...
  fbCoord = vec2(
    fbCoord.x + fbXDisplace + vX * VVV + 0.5,
 ...
 */
  ...
  fbColorHsb.x = abs(
    fbColorHsb.x * fbHue * (1.0 + vHue * VVV) +
      (fbHuexLfo + vHuexLfo * VVV) * sin(fbColorHsb.x / 3.14)
  );
  fbColorHsb.x = fract(
    mod(fbColorHsb.x, fbHuexMod + vHuexMod * VVV) + fbHuexOff + vHuexOff * VVV
  );
  //attenuate sat and bright
  fbColorHsb.y = clamp(
    fbColorHsb.y * fbSaturation * (1.0 + vSat * VVV),
    0.0,
    1.0
  );
  fbColorHsb.z = clamp(
    fbColorHsb.z * fbBright * (1.0 + vBright * VVV),
    0.0,
    1.0
  );
  ...
  fbColor = vec4(
    vec3(hsb2rgb(vec3(fbColorHsb.x, fbColorHsb.y, fbColorHsb.z))),
    1.0
  );

  //up the temporal Filter resonance
  vec3 temporalFilterColorHsb = rgb2hsb(
    vec3(temporalFilterColor.r, temporalFilterColor.g, temporalFilterColor.b)
  );
  temporalFilterColorHsb.z = clamp(
    temporalFilterColorHsb.z *
      (1.0 + 0.5 * temporalFilterResonance * (1.0 + vFb1X * VVV)),
    0.0,
    1.0
  );
  temporalFilterColorHsb.y = clamp(
    temporalFilterColorHsb.y *
      (1.0 + 0.25 * temporalFilterResonance * (1.0 + vFb1X * VVV)),
    0.0,
    1.0
  );
  temporalFilterColor = vec4(
    vec3(
      hsb2rgb(
        vec3(
          temporalFilterColorHsb.x,
          temporalFilterColorHsb.y,
          temporalFilterColorHsb.z
        )
      )
